...
const PLUGINS_DIR = path.join(process.cwd(), "commands")

async function loadCommandsAndPlugins(dir = PLUGINS_DIR) {
...
if (typeof imported.after === 'function' && !imported.default?.command) {
  global.middlewares.after.push(imported.after)
            ...
            if (!comando?.command || typeof comando.run !== "function") {
                console.log(chalk.yellow(`⚠ Archivo ${fileOrFolder} no es un comando válido (falta export default con command/run)`))
                continue
            }

            comando.command.forEach(cmd => {
                console.log(chalk.cyan(`✔ Comando cargado: ${cmd}`))
