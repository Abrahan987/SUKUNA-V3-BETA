...
import { resolveLidToRealJid } from "./functions.js"
...
export async function generateProfilePicture(buffer) {
  ...
  return {
    img: await cropped.resize({ width: 720, height: 720 }).getBuffer("image/jpeg"),
    preview: await cropped.resize({ width: 720, height: 720 }).getBuffer("image/jpeg"),
  }
}
...
export async function smsg(client, m, store) {
  ...
  m.copy = () =>
    smsg(client, proto.WebMessageInfo.fromObject(proto.WebMessageInfo.toObject(m)))
  ...
  m.react = (text, key, options) => client.sendMessage(m.chat, { react: { text,
key: m.key } })
...
const canalSeleccionado = canales[Math.floor(Math.random() * canales.length)]
  ...
  const pfp = botSetting.icon || ''
    ...
    const canalNam = botSetting.nameid || '──̄͟͞⛛̵̅𝐌̸͡𝐞𝐠̵𝐮̲͜𝐦̷̈𝐢͜𝐧̸̤𝐁̷𝐨̶̇͜𝐭𓊓̴̻𝐂̷𝐡̶͡𝐚𝐧̈͜ནེ͜͜ལ̵̲͞🔥̵̄͟'
      ...
      jid,
      {
        extendedTextMessage: {
          ...
          contextInfo: {
            ...
            externalAdReply: {
              title: namebugsito,
              body: `${namebugg2}, Built With 🔥 By SUKUNA`,
              mediaType: 1,
              renderLargerThumbnail: false,
              previewType: `PHOTO`,
              thumbnailUrl: pfp,
  ...
  client.sendContextInfoIndex = async (
    ...
    const namebot2 = config.body || `${settings.namebot2}, Built With 🔥 By SUKUNA`
    const canalId = settings.id ||'120363358338732714@newsletter'
    const canalName = settings.nameid || '──̄͟͞⛛̵̅𝐌̸͡𝐞𝐠̵𝐮̲͜𝐦̷̈𝐢͜𝐧̸̤𝐁̷𝐨̶̇͜𝐭𓊓̴̻𝐂̷𝐡̶͡𝐚𝐧̈͜ནེ͜͜ལ̵̲͞🔥̵̄͟'
    const sourceUrl =
